...
## timecourse data is also handled. In addition, pattern discovery is available- in which the eigengenes
...
##  also returns permuted values: foldchange.star,  ystar, sdstar, censoring.statusstar (for survival data)
...
censoring.status.star.keep <-  matrix(0,ncol=nperms.act,nrow=length(y))}
...
# ystar= xl.prevfit$ystar
 ...
 censoring.status.star.keep=xl.prevfit$censoring.status.star.keep
...
if(xl.mode=="regular"){
  ...
  for(b in first:last){
    ...
    if(resp.type==samr.const.survival.response){
      o <- perms[b,]
      ttstar[,b] <- cox.func(xstar,y[o],censoring.status=censoring.status[o],s0=s0)$tt


      censoring.status.star.keep[,b] <- censoring.status[o]
...
if(xl.mode=="regular" | xl.mode=="lasttime"){
  ...
  censoring.statusstar <- censoring.status.star.keep
  sdstar <- sdstar.keep
...
if(resp.type==samr.const.multiclass.response){
  ...
  # the last time through, unless otherwise specified, we delete x, since it is very big and is not needed further

if(return.x==FALSE){x=NULL}
}
...
censoring.status.star.keep=censoring.status.star.keep,
resp.type=resp.type,
